...
class nggdb {
    ...
    */
    function delete_image( $id ) {
        ...
        if ($result) {
          $result = $wpdb->query( $wpdb->prepare( "DELETE FROM $wpdb->nggmetadata WHERE pid = %d", $id) );
        }
    ...
    function update_image_meta( $id, $new_values ) {
        ...
        // Use cache object
        $gid = $wpdb->get_var( $wpdb->prepare( "SELECT galleryid FROM $wpdb->nggpictures WHERE pid = %d ", $id ) );
