...
function get_browse_meta() {
  global $wpdb;
	...
	if (!empty($_GET['search'])) {
	  ...
	  foreach ($keys as $key) {
	    $output .= '<h3>'. ucfirst($key) .'</h3>';
	    $query = "SELECT meta_value, COUNT(meta_value) AS count FROM $wpdb->nggmetadata WHERE meta_key = '{$key}' GROUP BY meta_value ORDER BY meta_value ASC";
	  }
	}
	
  if (!empty($_GET['by'])) {
    $query = "SELECT meta_value, COUNT(meta_value) AS count FROM $wpdb->nggmetadata WHERE meta_key = '". $_GET['by'] . "' GROUP BY meta_value ORDER BY meta_value ASC";
    $result = $wpdb->get_results( $query );
    $output .= '<ul>';
    $output .= '  <h3>By '. $_GET['by'] .'</h3>';
    foreach ($result as $meta) {
      $value = empty($meta->meta_value) ? 'None' : $meta->meta_value;
      $key = str_replace('_', ' ', ucfirst($value));
      $output .= '<li><a href="'. get_permalink(get_browse_pageid()) .'?by='. $_GET['by'] .'&value='. $value .'">'. $key .'</a> ('. parse_unit($meta->count, 'picture') .')</li>';
    }
    $output .= '</ul>';
	}
...
function get_browse_search_box() {
  $output .= '<form method="get">';
  if (is_numeric($_GET['page_id'])) {
    $output .= '<input type="hidden" name="page_id" value="'. $_GET['page_id'] .'" />';
  }
  $output .= '<input type="text" name="search" id="search" value="'. $_GET['search'] .'" />';
  $output .= '<input type="submit" value="Search" /></form>';
	return $output;
}

function parse_unit($quantity, $unit) {
  return $quantity .' '. ($quantity > 1 ? $unit.'s' : $unit);
}

function get_browse_sidebar() {
  global $wpdb;
  
  $excludes = array('width', 'height', 'credit', 'keywords', 'flash', 'created_timestamp', 'caption', 'title');
  
  $query = "SELECT DISTINCT(meta_key) FROM $wpdb->nggmetadata ORDER BY meta_key ASC";
  $result = $wpdb->get_results( $query );
  $output .= '<ul>';
  $output .= '  <li><h2>Browse pictures by</h2>';
  $output .= '    <ul>';
  foreach ($result as $meta) {
    //if (in_array($meta->meta_key, $excludes)) continue;
    $key = str_replace('_', ' ', ucfirst($meta->meta_key));
    if (trim($_GET['key']) == trim($meta->meta_key)) {
      $classes[] = 'active';
    }
    if (count($classes)) {
      $classes = implode(' ', $classes);
    }
    $output .= '<li><a href="'. get_permalink(get_browse_pageid()) .'?by='. $meta->meta_key .'" class="'. $classes .'">'. $key .'</a></li>';
    unset($classes);
  }
  $output .= '    </ul>';
  $output .= '  </li>';
...
function get_browse_results() {
  ...
  elseif ($_GET['search'] != '') {
    $s = $_GET['search'];
    $where = "((m.meta_key = 'title' AND m.meta_value LIKE '%{$s}%')";
    $where .= " OR (m.meta_key = 'caption' AND m.meta_value LIKE '%{$s}%')";
    $where .= " OR (m.meta_key = 'keywords' AND m.meta_value LIKE '%{$s}%'))";
    if ($_GET['key'] != '' && $_GET['value'] != '') {
      $value = $_GET['value'] == 'None' ? '' : $_GET['value'];
      //$where .= " AND (m.meta_key = '". $_GET['key'] ."' AND m.meta_value = '". $value ."')";
    }
    $query = "SELECT DISTINCT g.*, p.* FROM $wpdb->nggallery g, $wpdb->nggpictures p, $wpdb->nggmetadata m WHERE g.gid = p.galleryid AND p.pid = m.pid AND {$where}";
  }
  ...
  if ($result) {
    ...
    foreach ($result as $image) {
      if ($_GET['key'] != '') {
        $metas = unserialize($image->meta_data);
        $metas['description'] = $metas['caption'];
        $_value = $_GET['value'] == 'None' ? '' : $_GET['value'];
        foreach ($metas as $key => $value) {
           if (trim($key) == trim($_GET['key']) && trim($value) == trim($_value)) {
             $images[$image->pid] = new nggImage( $image );
           }
        }
      }
      else {
    ...
    if (!empty($_GET['search'])) {
      ...
      foreach ($keys as $key) {
        ...
        $result = $wpdb->get_results( $query );
        $output .= '<ul>';
        $output .= '  <h3>'. ucfirst(str_replace('_' ,' ', $key)) .'</h3>';
        if (count($result)) {
          foreach ($result as $meta) {
            $value = empty($meta->meta_value) ? 'None' : $meta->meta_value;
            $_key = str_replace('_', ' ', ucfirst($value));
            $punc = $_GET['page_id'] ? '&' : '?';
...
function get_browse_content() {
  $search = get_browse_search_box();
  $meta = get_browse_meta();
  $images = get_browse_results();
  
  $title = empty($meta) ? 'All pictures' : 'Browse pictures';
  $title = '<h2>'. $title .'</h2>';
  
  print $title . $search . $meta . $images;
}

function get_browse_pageid() {
  global $wpdb;
  return $wpdb->get_var("SELECT post_id FROM $wpdb->postmeta WHERE meta_key = '_wp_page_template' AND meta_value = 'browse-pictures.php' ORDER BY post_id ASC LIMIT 0, 1");
}
?>
